...
class FetchKardexWorker(
    ...
    override suspend fun doWork(): Result {
        ...
        Log.d("SICENET_WORKER", "Iniciando FetchKardexWorker")
        ...
        val result = repository.getKardexRemote(cookie, lineamiento)

        return if (result.isSuccess) {
            val jsonKardex = Gson().toJson(result.getOrNull())
            val outputData = workDataOf("kardex_json" to jsonKardex)
            Result.success(outputData)
        } else {
            Log.e("SICENET_WORKER", "Error en FetchKardexWorker: ${result.exceptionOrNull()?.message}")
            Result.failure()
        }
    }
}
